...
public class AuthController {
    ...
    /**
     * Cette méthode retourne l'utilisateur actuellement connecté.
     * Elle vérifie si un utilisateur est présent dans la session HTTP.
     * Si aucun utilisateur n'est connecté, une exception est levée.
     * @param session la session HTTP contenant l'utilisateur connecté
     * @return l'objet User représentant l'utilisateur connecté
     * @throws UnauthorizedException si aucun utilisateur n'est connecté
     */
    @GetMapping("/me")
    public User me(HttpSession session) {
        return AuthUtil.checkLogin(session);
    }
